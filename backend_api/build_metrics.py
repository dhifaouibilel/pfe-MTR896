...
class ChangeMetricsCalculator:
    ...
    def count_project_age(self, change: pd.Series) -> float:
        ...
        logger.info(f'change created: {change.created}')
        ...
        logger.info(f'project age: {(change.created - project_creation_date).total_seconds()}')
        return (change['created'] - project_creation_date).total_seconds()
    
    
        
    def count_project_changes(self, change: pd.Series) -> int:
    ...
    def count_whole_changes(self, change: pd.Series) -> int:
    ...
    def count_projects_changes_deps(self, change: pd.Series) -> int:
    ...
    def count_avg_cro_proj_nbr(self, change: pd.Series) -> float:
        ...
        df_deps_sub['project'] = df_deps_sub.apply(
            lambda x: x['Target_repo'] if change['project'] == x['Source_repo'] else x['Source_repo'], 
            axis=1)
    ...
    def count_last_x_days_modified_unique_projects(self, change: pd.Series, days: int) -> int:
    ...
    def count_whole_changes_deps(self, change: pd.Series) -> int:
    ...
    def count_num_modified_file_dependent_changes(self, change: pd.Series, attr: str) -> str:
        ...
        for label, func in sub_operations.items():
            change[f"{label}{attr}"] = func(list(file_counts.values()))
        
        change[attr] = len([count for count in file_counts.values() if count > 0])
        pd.DataFrame({k: [v] for k, v in change.items()}).to_csv(
            osp.join('.', 'Files', 'Metrics', attr, f"{change['number']}.csv"))
        return change['number']
    ...
    @staticmethod
    def count_num_build(change: pd.Series, attr: str) -> int:
    ...
    def count_num_recent_branch_files_changes(self, change: pd.Series) -> int:
        ...
        res = res.explode('changed_files')
        return res.loc[res['changed_files'].isin(change['changed_files']), 'branch'].nunique()
    
    def count_type_changes(self, change: pd.Series, change_type: str) -> int:
    ...
    def process_complex_metric(self, main_dev_attr: List[str], attr: str) -> None:
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = [
                executor.submit(self.count_num_modified_file_dependent_changes, change, attr) 
                for _, change in self.df_changes[main_dev_attr + ['number']].iterchanges()
            ]
